...
public final class PulseNativeDraw {
    ...
    public static boolean tryDrawArrays(int mode, int first, int count) {
        ...
        try {
            ...
            if (VTextureSelector.getBoundTexture(0) == null) {
                PulseDiagnostics.infoOnce("native-draw-texture-missing", "Pulse native draw waiting for Vulkan texture unit 0");
                return false;
            }
            ...
            CacheKey key = CacheKey.of(layout, first, count, flipY, matrix);
            ByteBuffer vertices = cachedKey != null && cachedKey.equals(key) && cachedVertices != null
                    ...
                    : translate(layout, first, count, flipY, matrix);
            ...
            PulseDiagnostics.infoOnce("native-copy-draw", "Pulse GL_TRIANGLES x6 is using cached Vulkan native POSITION_TEX draw");
            return true;
    ...
    private static Layout findLayout() {
        ...
        for (PulseVertexState.Attribute attribute : attributes) {
            ...
            if (position == null && (name.contains("pos") || name.contains("vertex")) && (attribute.size() == 2 || attribute.size() == 3 || attribute.size() == 4)) position = attribute;
            if (uv == null && (name.contains("uv") || name.contains("tex") || name.contains("coord")) && attribute.size() >= 2) uv = attribute;
        }
        if (position == null) {
            for (PulseVertexState.Attribute attribute : attributes) {
                if (attribute.type() == PulseVertexState.GL_FLOAT && (attribute.size() == 2 || attribute.size() == 3 || attribute.size() == 4)) { position = attribute; break; }
            }
        }
        if (uv == null) {
            for (PulseVertexState.Attribute attribute : attributes) {
                if (attribute != position && attribute.type() == PulseVertexState.GL_FLOAT && attribute.size() == 2) { uv = attribute; break; }
            }
        }
        if (position == null || uv == null || position.bufferId() == 0 || uv.bufferId() == 0) return null;
        return new Layout(position, uv);
    }

    private static ByteBuffer translate(Layout layout, int first, int count, boolean flipY, ShaderFallback.Matrix4Value matrix) {
        ...
        boolean directClip = minX >= -1.25f && maxX <= 1.25f && minY >= -1.25f && maxY <= 1.25f && (maxX - minX >= 1.5f || maxY - minY >= 1.5f);
        boolean unitQuad = minX >= -0.1f && maxX <= 1.1f && minY >= -0.1f && maxY <= 1.1f && (maxX - minX >= 0.8f && maxY - minY >= 0.8f);
        if (matrix == null && !directClip && !unitQuad) {
            PulseDiagnostics.infoOnce("native-draw-transform-missing", "Pulse native draw captured vertex layout but needs a transform matrix; range x=" + minX + ".." + maxX + " y=" + minY + ".." + maxY);
            return null;
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!Float.isFinite(x) || !Float.isFinite(y) || !Float.isFinite(z) || Math.abs(x) > 8.0f || Math.abs(y) > 8.0f) return null;
            float u = tex[i][0];
    ...
    private record CacheKey(int posId, long posRevision, int uvId, long uvRevision, int posStride, long posPtr, int uvStride, long uvPtr, int first, int count, boolean flipY, int matrixHash) {
        static CacheKey of(Layout layout, int first, int count, boolean flipY, ShaderFallback.Matrix4Value matrix) {
            int matrixHash = matrix == null ? 0 : 31 * Arrays.hashCode(matrix.values()) + Boolean.hashCode(matrix.transpose());
            return new CacheKey(layout.position.bufferId(), PulseBufferFallback.revision(layout.position.bufferId()), layout.uv.bufferId(), PulseBufferFallback.revision(layout.uv.bufferId()), layout.position.effectiveStride(), layout.position.pointer(), layout.uv.effectiveStride(), layout.uv.pointer(), first, count, flipY, matrixHash);
        }
    }
}
